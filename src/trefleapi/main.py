...
class Client(QuerySet):
    def __init__(self, token: str = None, query: str = None, category: str = 'plants', request_type: str = "list"):
        super().__init__(query=query, category=category, request_type=request_type)
        self.token = token
        self.adapter = RestAdapter(token==self.token)
        self.routes = Routes(version=VERSION)
